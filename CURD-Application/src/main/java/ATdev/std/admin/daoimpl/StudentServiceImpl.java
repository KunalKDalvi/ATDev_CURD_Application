...
public class StudentServiceImpl implements StudentService{
	...
	public String addStudent(List<Student> lst) {
		...
		try {
			PreparedStatement pstate = con.prepareStatement("insert into StudentD values(?,?,?,?)");
	...
	public List<Student> retriveStudent(int stdId) {
		...
		try {
			PreparedStatement pstate = con.prepareStatement("select*from StudentD where stdId=?");
	...
	public boolean deleteStudent(int stdId) {
		...
		Connection con=stdObj.getConnection();
		try {
			PreparedStatement pstate=con.prepareStatement("delete from StudentD where stdId=?");
			pstate.setInt(1, stdId);
	...
	public List<Student> getAllStudent() {
		...
		try {
			PreparedStatement pstate=con.prepareStatement("select*from StudentD");
			ResultSet rs=pstate.executeQuery();
			stdlst=new ArrayList<Student>();
			while(rs.next()) {
				Student std=new Student(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getString(4));
